    ...
    <>
      <header className={"site-header " + (scrolled ? "is-scrolled" : "")}>
        <div className="site-header__inner">
          ...
          <button className={"burger " + (menuOpen ? "is-open" : "")} onClick={() => setMenuOpen(!menuOpen)} aria-label="Open menu">
      ...
      {menuOpen && (
        <div className="mobile-menu">
          ...
          <div className="mobile-menu__bottom">
            ...
            <button className="btn btn--ink" onClick={() => { onReserveClick(); setMenuOpen(false); }}>
              {t.nav.reserve}
            </button>
          </div>
        </div>
    ...
    <section id="top" className="hero">
      <div className="hero__inner">
        ...
        <div className="hero__visual">
          <img src="assets/hero.jpeg" alt="Garden Resto" className="hero__img" />
        </div>
      </div>
    </section>
  );
    ...
    <section id="atmosphere">
      <div className="wrap">
        ...
        <div className="gallery">
          <div className="gallery__item"><img src="assets/dining.jpeg" alt="Dining room" /></div>
          <div className="gallery__item"><img src="assets/coffee.jpeg" alt="Coffee corner" /></div>
          <div className="gallery__item"><img src="assets/breakfast.jpeg" alt="Breakfast" /></div>
          <div className="gallery__item"><img src="assets/window-seat.jpeg" alt="Window seat" /></div>
          <div className="gallery__item"><img src="assets/guests.jpeg" alt="Guests" /></div>
        </div>
      </div>
    </section>
  );
      ...
      <div className="wrap">
        ...
        <div className="desserts__intro">
          ...
          <div className="story__portrait" style={{ aspectRatio: "4/5" }}>
            <img src="assets/dessert-counter.jpeg" alt="Tatiana's desserts" className="story__portrait-img" />
          </div>
        </div>
    ...
    <section id="events" className="events" ref={formRef}>
      <div className="wrap events__inner">
        <div>
          ...
          <h2 className="display events__heading"><em>{t.events.heading}</em></h2>
        ...
        <form className="form" onSubmit={submit}>
          ...
          <div className="field"><label>{t.events.fields.name}</label><input type="text" required value={data.name} onChange={onChange("name")} /></div>
          <div className="field"><label>{t.events.fields.contact}</label><input type="text" required value={data.contact} onChange={onChange("contact")} /></div>
          <div className="field-row">
            <div className="field"><label>{t.events.fields.date}</label><input type="text" placeholder="2026-06-12" value={data.date} onChange={onChange("date")} /></div>
            <div className="field"><label>{t.events.fields.guests}</label><input type="number" min="2" max="40" value={data.guests} onChange={onChange("guests")} /></div>
          </div>
          <div className="field"><label>{t.events.fields.details}</label><textarea rows="3" value={data.details} onChange={onChange("details")} /></div>
          <button type="submit" className="btn btn--terracotta">{t.events.fields.submit} <span className="arrow">→</span></button>
        </form>
      </div>
    </section>
  );
    ...
    <section id="contact" className="contact">
      <div className="wrap">
        ...
        <div className="contact__head">
          <h2 className="display contact__heading"><em>{t.contact.heading}</em></h2>
        </div>
        <div className="contact__grid">
          <a className="map-card" href="https://share.google/rkFq91LqIWDjCMQh1" target="_blank" rel="noopener" aria-label="Open in Google Maps">
            <iframe title="Map" src="https://maps.google.com/maps?q=Vana-Kalamaja+7a,+Tallinn,+Estonia&hl=en&z=17&output=embed" loading="lazy"></iframe>
          ...
          <div className="contact__col">
            <div>
              ...
              <div className="contact__address">{t.contact.address.map((a, i) => <p key={i}>{a}</p>)}</div>
            </div>
            <div>
              ...
              <table className="hours-table">
                <tbody>
                  {t.contact.hours.map((row, i) => (
                    <tr key={i} className={i === todayIdx ? "is-today" : ""}><td>{row.d}</td><td>{row.h}</td></tr>
                  ))}
                </tbody>
              </table>
            ...
            <div>
              ...
              <div className="socials">
                <a href="https://www.instagram.com/gardenresto.tallinn/" target="_blank" rel="noopener">Instagram</a>
                <a href="https://www.facebook.com/gardenresto.tallinn" target="_blank" rel="noopener">Facebook</a>
              </div>
            </div>
          </div>
        </div>
        <div className="contact__cta-row">
          ...
          <button className="btn btn--ink" onClick={onReserve}>{t.contact.reserveCta} <span className="arrow">→</span></button>
        </div>
  ...
  return (
    <TweaksPanel>
      <TweakSection title="Button shape">
        <TweakRadio value={tweaks.buttonShape} onChange={(v) => setTweak("buttonShape", v)}
          options={[{ label: "Pill", value: "pill" }, { label: "Rounded", value: "rounded" }, { label: "Square", value: "square" }]} />
      </TweakSection>
    </TweaksPanel>
  );
}
